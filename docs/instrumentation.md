# OpenTelemetry Instrumentation in Smith-Agent-Framework

This document provides an overview of the OpenTelemetry instrumentation setup in the Smith-Agent-Framework.
...
Use the environment variable `Smith_FRAMEWORK_INSTRUMENTATION_ENABLED` to enable or disable instrumentation.

```bash
# Enable instrumentation
export Smith_FRAMEWORK_INSTRUMENTATION_ENABLED=true
# Ignore sensitive keys from collected events data
export INSTRUMENTATION_IGNORED_KEYS="apiToken,accessToken"
```

If `Smith_FRAMEWORK_INSTRUMENTATION_ENABLED` is false or unset, the framework will run without instrumentation.
...
const tracer = trace.getTracer("Smith-agent-framework");
...
Running the Instrumented Application (`examples/agents/Smith_instrumentation.js`) file.

```bash
## the telemetry example is run on built js files
yarn start:telemetry ./examples/agents/Smith_instrumentation.ts
```
