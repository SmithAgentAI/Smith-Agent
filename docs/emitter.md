...
> Location within the framework `Smith-agent-framework/emitter`.
...
```ts
import { Emitter, EventMeta } from "Smith-agent-framework/emitter/emitter";
...
```ts
import { Emitter, EventMeta, Callback } from "Smith-agent-framework/emitter/emitter";
...
await emitter.emit("update", { id: 123, data: "Hello Smith!" });
```
...
import { BaseLLM } from "Smith-agent-framework/llms/base";
...
> [!TIP]
>
> The more complex agentic example can be found [here](/examples/agents/Smith.ts).
