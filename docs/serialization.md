...
> [!TIP]
>
> Location within the framework `smith-agent-framework/serializer`.
...
```ts
import { Serializer } from "smith-agent-framework/serializer/serializer";
...
const instance = new MyClass("smith");
...
```ts
import { Serializable } from "smith-agent-framework/internals/serializable";
