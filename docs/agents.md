...
## Implementation in Smith Agent Framework
...
Our Smith Agent is based on the `ReAct` ([Reason and Act](https://arxiv.org/abs/2210.03629)) approach.
...
const agent = new SmithAgent({
  llm: new OllamaChatLLM(), // for more explore 'Smith-agent-framework/adapters'
  memory: new UnconstrainedMemory(), // for more explore 'Smith-agent-framework/memory'
  tools: [new OpenMeteoTool()], // for more explore 'Smith-agent-framework/tools'
});
...
Please refer to the [following example](/examples/agents/Smith_advanced.ts) to see how to modify them.
